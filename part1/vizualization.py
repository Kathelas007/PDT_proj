...
def extreme_ration_table(df_in: pd.DataFrame, table_location):
    """
    Task number 4 - table
    Get table of extreme, neutral and total count of conspiracy theories for every.
    """
    teory = df_in['teory'].unique()
    for t in teory:
        df = df_in[df_in['teory'] == t]
        ...
        df.to_markdown(f"{table_location[0:-3]}_{t}.md")


def extreme_ration_plot(df: pd.DataFrame, teories: list, fig_location: str = None, show_figure: bool = False):
    """
    Task number 4 - plot
    ...
    # plot extreme count by theory
    fig, axes = plt.subplots(7, 2, figsize=(8.27, 11.69), sharex=True)
    axes = axes.flatten()

    theories = df['teory'].unique()
    for i in range(0, 14):
        sns.lineplot(data= df[df['teory'] == theories[i]], x='date', y='extreme', ax=axes[i])
        axes[i].set_title(theories[i])
        axes[i].tick_params(axis='x', labelrotation=21)
...
def extreme_ration(teories: list, table_location, plot_location):
    """ Task number 4 """
    ...
    extreme_ration_plot(df, teories, plot_location, show_figure=True)
    extreme_ration_table(df, table_location)


def active_accounts(teories, table_location):
    """ Task number 5 """

    tables = [f'    \n\n###{teory}   \n\n' + db.get_extreme_accounts(teory).set_index('id').to_markdown()
              for teory in teories]

    with open(table_location, 'w') as fd:
        fd.write('  \n'.join(tables))


def extreme_hashtags(teories, table_location):
    """ Task number 6 """
...
def visualize(conspiracy_teories: dict):
    ...
    extreme_ration(teories_only, table_location='./doc/extreme_tweet_cnt.md', plot_location='./img/tweet_plot.jpg')
    active_accounts(teories_only, './doc/extreme_accounts.md')
    extreme_hashtags(conspiracy_teories, './doc/extreme_hashtags.md')
